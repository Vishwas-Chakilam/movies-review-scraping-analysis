...
def analyze_data(df):
    ...
    plt.grid(True)
    plt.savefig('analysis/ratings_distribution.png')
    plt.show()
    ...
    if 'genre' in df.columns:
        ...
        plt.grid(True)
        plt.savefig('analysis/movies_by_genre.png')
        plt.show()
    ...
    plt.ylabel('Number of Movies')
    plt.grid(True)
    plt.savefig('analysis/movies_by_year.png')
    plt.show()

    # Top Rated Movies
    top_rated = df.sort_values(by='rating', ascending=False).head(10)
    print("Top Rated Movies:")
    print(top_rated[['movie_title', 'rating']])
